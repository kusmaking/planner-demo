(() => {
    ...
    wrapper.innerHTML = `
      <div id="personalBlockCard" class="rounded-2xl bg-white border border-slate-200 shadow-sm h-full">
        <div class="p-4 border-b border-slate-200">
          <h2 class="font-semibold">Fravær og intern aktivitet</h2>
          <p class="text-sm text-slate-500 mt-1">Brukes for kurs, ferie, syk og avspasering direkte på personen, uten å gå via prosjekt.</p>
        </div>
  ...
  function ensureCalendarAuditPanel() {
    const calendarSection = els.tabCalendarSection;
    const auditCard = getCardByElement(els.auditList);
    if (!calendarSection || !auditCard) return;

    calendarSection.classList.add("xl:flex-wrap");
    auditCard.classList.add("w-full", "xl:basis-full");

    if (auditCard.parentElement !== calendarSection) {
      calendarSection.appendChild(auditCard);
    }
  }
  ...
  function ensureCalendarContextMenu() {
    ...
    menu.innerHTML = `
      <div class="p-4 border-b border-slate-200 flex items-center justify-between gap-3">
        <div>
          <div class="font-semibold">Legg til fravær / aktivitet</div>
          <div class="text-xs text-slate-500 mt-1">Fra høyreklikk i kalenderen</div>
        </div>
  ...
  function renderLegend() {
    ...
    els.legendList.innerHTML = `
      ...
      <div class="pt-4 border-t border-slate-200">
        <div class="font-medium mb-2">Fravær og intern aktivitet</div>
