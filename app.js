(() => {
  // v18.23-startscreen-dashboard-safe
  ...
  function cacheElements() {
    const ids = [
      ...
      "saveStatus", "plannerTabs", "tabHomeBtn", "tabCalendarBtn", "tabProjectPlanBtn", "tabUnstaffedBtn", "tabProjectsBtn", "tabEmployeesBtn", "tabAdminBtn", "tabHomeSection", "homeDashboard", "tabCalendarSection", "tabProjectsSection", "tabEmployeesSection", "tabAdminSection", "calendarMainCol", "calendarPanelCol", "calendarPanelHandleBtn", "calendarPanelCloseBtn", "calendarPanelContent", "newProjectBtn", "projectModal", "projectModalTitle", "closeProjectModalBtn",
  ...
  function getActivePlannerNavKey() {
  ...
  function renderLayoutTabs() {
    ...
    const activeNavKey = getActivePlannerNavKey();
    const buttons = {
      home: els.tabHomeBtn,
    ...
    Object.entries(buttons).forEach(([name, btn]) => {
      ...
      if (activeNavKey === name) btn.setAttribute("aria-current", "page");
...
async function deleteEditedEntry() {
  ...
  function renderHomeDashboard() {
    ...
    const busyEmployees = Array.from(busyNames).map(name => employeeNameMap.get(name)).filter(Boolean);
    ...
    const groupRows = getOrderedEmployeeGroups().map(group => {
    ...
    const homeActionIcon = (key) => {
      const attrs = 'viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"';
      const icons = {
        calendar: `<svg ${attrs}><rect x="3" y="4" width="18" height="18" rx="2"/><path d="M16 2v4M8 2v4M3 10h18"/></svg>`,
        briefcase: `<svg ${attrs}><path d="M8 7V5a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/><rect x="3" y="7" width="18" height="13" rx="2"/><path d="M3 12h18"/></svg>`,
    ...
    const shortcuts = [
      ...
      { key: 'briefcase', title: 'Prosjektplan', text: 'Planlegg prosjekter og tildel oppdrag.', action: 'project' },
    ...
    const firstName = String(getAccountDisplayName() || state.currentUser || 'Planlegger').trim().split(/\s+/)[0] || 'Planlegger';
    const groupHtml = groupRows.map(row => `
    ...
    els.homeDashboard.innerHTML = `
      <div class="space-y-5">
        ...
        <div class="rounded-[28px] bg-white border border-slate-200 shadow-sm p-6 flex items-center gap-5">
          <span class="inline-flex h-20 w-20 items-center justify-center rounded-full bg-cyan-50 text-cyan-700 shrink-0">${homeActionIcon('sun')}</span>
          <div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-4">
          ${shortcuts.map(card => `
            <button type="button" data-home-action="${card.action}" class="text-left rounded-[28px] bg-white border border-slate-200 shadow-sm p-6 min-h-[220px] hover:border-slate-300 hover:bg-slate-50 transition">
              <span class="inline-flex h-16 w-16 items-center justify-center rounded-full bg-slate-50 text-slate-700">${homeActionIcon(card.key)}</span>
        ...
        <div class="grid grid-cols-1 xl:grid-cols-12 gap-4">
          <div class="xl:col-span-4 rounded-[28px] bg-white border border-slate-200 shadow-sm p-5">
            ...
            <div class="mt-6 flex items-center gap-6">
              <div class="relative h-40 w-40 shrink-0 rounded-full" style="background: conic-gradient(#34d3bf 0 ${utilizationPct}%, #e5e7eb ${utilizationPct}% 100%);">
                <div class="absolute inset-[18px] rounded-full bg-white flex flex-col items-center justify-center text-center">
                  <div class="text-5xl font-semibold text-slate-950">${utilizationPct}%</div>
                  <div class="mt-2 text-base text-slate-600">Total utnyttelse</div>
                </div>
              </div>
              <div class="space-y-3 text-base text-slate-700">
                <div><span class="font-semibold text-slate-950">Belagt:</span> ${busyCount} ansatte</div>
                <div><span class="font-semibold text-slate-950">Tilgjengelig:</span> ${availableCount} ansatte</div>
          ...
          <div class="xl:col-span-4 rounded-[28px] bg-white border border-slate-200 shadow-sm p-5">
            ...
            <div class="mt-4">${groupHtml || '<div class="py-8 text-sm text-slate-500">Ingen ansatte tilgjengelig.</div>'}</div>
          </div>

          <div class="xl:col-span-4 rounded-[28px] bg-white border border-slate-200 shadow-sm p-5">
            <div class="flex items-center justify-between gap-3">
              <h3 class="text-[20px] font-semibold text-slate-950">Dagens oversikt</h3>
              <span class="text-sm text-slate-400">${escapeHtml(formatDate(today))}</span>
            </div>
            <div class="mt-4 divide-y divide-slate-200">
              <div class="flex items-center justify-between gap-3 py-4"><span class="text-base text-slate-700">Aktive prosjekter</span><span class="text-4xl font-semibold text-slate-950">${activeProjects.length}</span></div>
              <div class="flex items-center justify-between gap-3 py-4"><span class="text-base text-slate-700">Prosjekter uten bemanning</span><span class="text-4xl font-semibold text-slate-950">${unstaffedCount}</span></div>
              <div class="flex items-center justify-between gap-3 py-4"><span class="text-base text-slate-700">Tilgjengelige ansatte</span><span class="text-4xl font-semibold text-slate-950">${availableCount}</span></div>
            </div>
          </div>
        </div>
      </div>
