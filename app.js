            ...
            <div class="grid grid-cols-[1fr_auto] items-center gap-2 rounded-2xl border border-slate-200 bg-white px-3 py-2">
              <div class="min-w-0">
                <div class="flex min-w-0 items-center gap-2">
                  ...
                  <button data-project-entry-edit-id="${escapeHtml(entry.id)}" type="button" class="shrink-0 border border-cyan-200 bg-cyan-50 px-2 py-1 text-[11px] font-semibold text-cyan-700 hover:bg-cyan-100">Endre</button>
                </div>
                <div class="mt-1 text-[11px] text-slate-500">${escapeHtml(entry.role || "Rolle ikke satt")} · ${escapeHtml(formatDate(entry.start_date))} – ${escapeHtml(formatDate(entry.end_date))}</div>
              </div>
              <button data-project-entry-delete-id="${escapeHtml(entry.id)}" type="button" class="px-2 py-1 text-slate-400 hover:text-red-600" title="Fjern tildeling">×</button>
            </div>
