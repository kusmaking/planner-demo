(() => {
  ...
  function renderAssignSummary(project) {
    ...
    const startText = project.planned_start_date ? formatDate(project.planned_start_date) : "Ingen dato";
    const endText = project.planned_end_date ? formatDate(project.planned_end_date) : "Ingen dato";
    els.assignSummary.textContent = `${project.name} • Behov: ${required} person${required > 1 ? "er" : ""} • ${startText} – ${endText}`;
  }

  function applyAssignProjectDates(project) {
  ...
  function syncAssignDatesFromProject() {
    ...
    const previousState = getAssignFormState();
    renderAssignEmployeeSelectors(previousState.rows);
    applyAssignProjectDates(project);
    renderAssignSummary(project);
  }
  ...
  function renderAssignEmployeeSelectors(preservedRows = null) {
    if (!els.assignEmployeesWrap) return;
    const project = getProjectById(els.assignProject?.value);
    const activeEmployees = state.employees.filter(e => e.active !== false);
    const currentRows = Array.isArray(preservedRows)
      ? preservedRows
      : Array.from(els.assignEmployeesWrap.querySelectorAll("[data-assign-row]")).map((row, index) => ({
          employee_name: row.querySelector("[data-assign-employee-select]")?.value || "",
          role: row.querySelector("[data-assign-role-select]")?.value || getDefaultRoleForIndex(index)
        }));
    const count = Math.max(Number(project?.headcount_required || 0), currentRows.length, 1);
  ...
  function populateDynamicSelects() {
    ...
    const assignState = getAssignFormState();
    const selectedAssignProjectId = visibleProjects.some(project => project.id === assignState.projectId)
    ...
    renderAssignEmployeeSelectors(selectedAssignProjectId ? assignState.rows : []);
    ...
    applyAssignProjectDates(selectedProject);
    if (els.assignNotes) els.assignNotes.value = assignState.notes || "";
    renderAssignSummary(selectedProject);
  }
